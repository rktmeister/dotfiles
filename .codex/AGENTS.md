...
Do not read the `.env` file unless you are given permission by the user to do so.

- When calling `https://attachments.chatforgoodtech.com/`, include the headers `CF-Access-Client-Id: $CF_ACCESS_CLIENT_ID` and `CF-Access-Client-Secret: $CF_ACCESS_CLIENT_NOT_ID`.
- Never log, print, or otherwise expose the values of `$CF_ACCESS_CLIENT_ID` or `$CF_ACCESS_CLIENT_NOT_ID`.
...
Always use `uv` (unless you are instructed to do otherwise) when dealing with python projects. here are some common commands to use:
...
Always use `pnpm` as the package manager (unless you are instructed to do otherwise) when dealing with Typescript / Javascript projects. Here are some common commands:
